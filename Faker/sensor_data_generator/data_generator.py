...
class SensorDataGenerator(object):
            ...
            for params in params_list:
                ...
                if "frm" in params.keys():
                    if params["frm"] >= self.sample_size or frm < 0:
                        raise ValueError("invalid range: 0 <= frm < to <= sample_size! frm:",
                                         params["frm"], "sample size:", self.sample_size)
                    else:
                        frm = params["frm"]
                ...
                if "to" in params.keys():
                    if params["to"] > self.sample_size or params["to"] <= 0:
                        raise ValueError("invalid range: 0 <= frm < to <= sample_size! to:",
                                         params["to"], "sample size:", self.sample_size)
                    else:
                        to = params["to"]

                frm, to = int(frm), int(to)
                if frm >= to:
                    raise ValueError("invalid range: 0 <= frm < to <= sample_size! frm, to:",
                                     frm, to, "sample size:", self.sample_size)
                ...
                if "distribution" in params.keys() or "copula" in params.keys():
                    sens_generator_mapping[sname].append(
                        st(sname=sname, params=params, sample_size=sample_size))
                elif "attractor" in params.keys():
                    sens_generator_mapping[sname].append(
                        at(sname=sname, params=params, sample_size=sample_size))
                elif "eq" in params.keys():
                    sens_generator_mapping[sname].append(
                        ce(sname=sname, params=params, sample_size=sample_size))
                elif "at_eq" in params.keys():
                    sens_generator_mapping[sname].append(
                        cat(sname=sname, params=params, sample_size=sample_size))
        ...
        if frm is None:
            ...
            if not isinstance(frm, int):
                ...
                if frm >= max_size or frm < 0:
                    raise ValueError(
                        "frm must be: 0 <= frm < to <= "+str(max_size)+" !")
        ...
        if to is None:
            ...
            if not isinstance(to, int):
                ...
                if to > max_size or to < 0 or to <= frm:
                    raise ValueError(
                        "to must be: 0 <= frm < to <= "+str(max_size)+" !")
        return frm, to
    ...
    def __check_sensor(self, sensors=[]):
        ...
        if sensors is None:
            raise ValueError(
                "please specify sensor name(s) in list form! ex) [\"Sensor1\", \"Sensor2\", ...]")
    ...
    def sum(self, sensors=[], save_to=None, frm=None, to=None):
        ...
        sensors, save_to, frm, to = self.__check_calc_params(
            sensors, save_to, frm, to)
    ...
    def sub(self, sensors=[], save_to=None, frm=None, to=None):
        ...
        sensors, save_to, frm, to = self.__check_calc_params(
            sensors, save_to, frm, to)
        new_data = self.data[sensors[0]][frm:to].sub(
            self.data[sensors[1:]][frm:to].sum(axis=1))
        self.__assign_new_data(save_to, frm, to, new_data)
    ...
    def load_from_excel(self, file_name="", sheet_name=""):
        ...
        self.data = pd.read_excel(
            io=open(file_name, 'rb'), sheet_name=sheet_name)
        self.sample_size = self.data.shape[0]
        return self.data
    ...
    def plot_data(self, file_name=None, sensors=[], just_save=False):
        ...
        ax.legend(loc='center right', bbox_to_anchor=(
            1.12, 0.5), shadow=True, ncol=1)
        num_data = len(self.data)
        amount = int(0.2*self.data.shape[0])
        loc = [i for i in range(num_data) if i % amount ==
               0 and i >= 0 and i < num_data or i == num_data-1]
        stamp = [self.data.index.values[i] for i in range(
            num_data) if i % amount == 0 and i >= 0 and i < num_data or i == num_data-1]
    ...
    def generate(self, sample_size=1000, resize=False, seed=None):
        ...
        if not isinstance(sample_size, int) or sample_size <= 0:
            raise ValueError(
                "sample size must be an integer and greater than 0")
        if self.data is None and self.sample_size is None:
            raise ValueError(
                "sample size must be defined for initial generation!")
        if resize and self.sample_size != sample_size:
            if self.data is None:
                raise ValueError(
                    "data table does not exist! please generate numbers first!")
            if self.sample_size > sample_size:
                ...
                for col in list(self.data.columns):
                    ...
                    col_data = np.append(
                        col_data, np.full(additional_num_rows, None))
        ...
        # data generation
        sens_generator_mapping, sens_frm_to_mapping = self.__initialize(
            input=self.generation_input.options)
        sens_data_mapping = self.__generate_from_generator(
            sens_generator_mapping, seed)
        temp_data_table = {}
    ...
    def generate_covmat(self, dim=2, sigmas=None, corrs=None, eigs=None, seed=None):
        ...
        if sigmas is not None:
            if (np.array(sigmas) < 0.0).all():
                raise ValueError(
                    "all sigmas must be greater than or equal to 0.")
            if len(sigmas) != dim:
                raise ValueError(
                    "the length of sigmas must be equal to \"dim\". length of sigmas:", len(sigmas))
        else:
            sigmas = np.full(dim, 1.0)
        if corrs is not None:
            if not isinstance(corrs, np.ndarray) and corrs.shape != (dim, dim):
                raise ValueError(
                    "corrs must be a \"dim\" x \"dim\" numpy matrix.")
            if (-1.0 > corrs).all() or (corrs > 1.0).all():
                raise ValueError(
                    "each correlation value must be in [-1.0, 1.0].")
        else:
            self.seed(seed)
            if eigs is None:
    ...
    def multinomial_process(self, sensors=[], pval=[], sample_wise=False, save_to="MP_SENSOR", keep_old=True, seed=None, frm=None, to=None):
        ...
        if not sample_wise:
            if len(sensors) != len(pval):
                raise ValueError(
                    "incorrect number of sensor name and probability input:", sensors, pval)
            if np.sum(pval) != 1:
            ...
            if pval.shape[0] != desired_size:
                raise ValueError(
                    "the number of pval samples is not equal to the desired modification size!", pval.shape[0], desired_size)
            for pv in pval:
                if np.sum(pv) != 1:
                    raise ValueError("incorrect probability input:", pval)
    ...
    def replace(self, source=[], destination=[], frm=None, to=None):
        ...
        for i in range(len_destination):
            ...
            self.__assign_new_data(
                destination[i], frm, to, self.data[source[index]][frm:to])
    ...
    def reverse(self, sensors=[], axis=0, frm=None, to=None):
        ...
        if axis == 0:
            if frm_idx == 0 and to_idx == self.sample_size:
                ...
                self.data.loc[frm:to, sensors] = self.data.loc[frm:to,
                                                               sensors][::-1].values
    ...
    def weighted_sum(self, sensors=[], weight=[], sample_wise=False, save_to=None, frm=None, to=None):
        ...
        if sample_wise:
            if weight.shape[0] != int(to-frm) or weight.shape[1] != len(sensors):
                raise ValueError("shape of weight must be:",
                                 frm-to, len(sensors))
        else:
            if len(sensors) != len(weight):
                raise ValueError(
                    "all sensors must have its own weight assigned!")

        destination = sensors[0] if save_to is None else save_to
        new_data = np.multiply(
            np.array(self.data[sensors][frm:to]), weight).sum(axis=1)
