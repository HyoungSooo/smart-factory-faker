...
class CustomAttractor(AbstractGenerator):
    ...
    def __init__(self, sname="", params={}, sample_size=1):
        ...
        if len(params["initial"].keys()) != len(self.variables):
            out = "must include step and initial term for all variables: " + \
                str(self.variables)
    ...
    def __generate_custom_attractor(self):
        ...
        lambdified_f = lambdify(
            self.variables, self.functions, modules=["numpy"])
        ...
        # Initialize
        initial_state = [float(self.params["initial"][str(var)])
                         for var in self.variables]
        frm, dt = float(self.params["start_t"]), float(
        ...
        for i in range(x.shape[1]):
            self.sens_gen_option_mapping[self.sname[i]
                                         ] = "Custom_Attractor_"+str(i)
